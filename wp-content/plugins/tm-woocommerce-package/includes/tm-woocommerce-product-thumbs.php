...
if ( ! class_exists( 'Tm_Woo_Product_Thumbnails' ) ) {
	...
	class Tm_Woo_Product_Thumbnails {
		...
		function __construct() {
			...
			if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '3.4.7', '>' ) ) {
				add_filter( 'woocommerce_product_get_image', array( $this, 'advanced_wc_product_thumb_image' ), 20, 6 );
			}else{
				add_filter( 'post_thumbnail_html', array( $this, 'advanced_thumb' ), 20, 5 );
			}
		 ...
		 * @return [type]               [description]
		 */
		public function advanced_wc_product_thumb_image( $html, $post, $size, $attr, $placeholder, $image ) {

			if ( ! $this->is_thumb_required( $post->get_id() ) ) {
				return $html;
			}
